...
public class UserRelationshipServiceTest {
  ...
  @InjectMocks private UserRelationshipServiceImpl userRelationshipService;
  ...
  void testAcceptFriendRequest_ShouldCreateMutualFriendship() {
    ...
    UserRelation originalRelation =
        savedRelations.stream()
  ...
  void testGetPendingFriendRequests_ReturnsOnlyPending() {
    ...
    when(userRelationRepository.findByFriendIdAndType(
            user2.getId(), UserRelation.RelationType.PEDIDO))
  ...
  void testGetRelationStatus_PrivacyWhenRejected() {
    UserRelation rejectedRequest =
        new UserRelation(user1, user2, UserRelation.RelationType.PEDIDO_REJEITADO);
  ...
  @Test
  void testRemoveFriendship_Success() {
    ...
    userRelationshipService.removeFriendship(user1.getId(), user2.getId());

    verify(userRelationRepository).delete(friendship1);
    verify(userRelationRepository).delete(friendship2);
    verify(notificationService, never()).sendNotification(any(), any());
  }

  @Test
  void testRemoveFriendship_FriendshipNotFound() {
    when(userRelationRepository.findRelationship(user1.getId(), user2.getId()))
        .thenReturn(Optional.empty());
  ...
  @Test
  void testRemoveFriendship_SelfRemoval() {
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> {
              userRelationshipService.removeFriendship(user1.getId(), user1.getId());
            });

    assertEquals("Não se pode remover a si mesmo da lista de amigos.", exception.getMessage());
  }
}
