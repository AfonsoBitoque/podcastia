...
\hypersetup{
  ...
  urlcolor   = accent,
  citecolor  = accent,
  pdftitle   = {Relatório de Projeto — Podcastia},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{gray}{Podcastia — Relatório de Projeto}}
...
a plataforma oferece funcionalidades sociais completas --- amizades, chat privado
...
\begin{description}
  \item[\texttt{PodcastGenerationService}] Implementa o pipeline completo de geração: chamada à Gemini API $\rightarrow$ limpeza do guião $\rightarrow$ síntese TTS $\rightarrow$ persistência do MP3.
  ...
  \item[\texttt{ChatMessageServiceImpl}] Gere o ciclo completo de mensagens: envio, entrega, leitura, reações e eliminação, com rate limiting e blacklist de links.
...
\begin{destaque}[title=Nota sobre desempenho]
  ...
  da dimensão do guião gerado e da velocidade da rede. Para podcasts com 3--5 minutos
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{4cm} L{2cm} X}
    ...
    H2 Database        & ---       & Base de dados para desenvolvimento e testes \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{4cm} L{2cm} X}
    ...
    WebSocket STOMP    & ---       & Chat privado em tempo real \\
...
\begin{itemize}
  \item \textbf{Tema} --- assunto do podcast em linguagem natural (ex: \emph{"Curiosidades sobre o espaço sideral"});
...
abordagem de \emph{defense in depth}:
...
\begin{itemize}
  ...
  \item \textbf{Programático}: operações sobre recursos (ex: editar ou eliminar uma playlist) verificam programaticamente se o utilizador autenticado é o proprietário do recurso.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{window.confirm()} bloqueado no browser} (médio) --- o diálogo nativo é bloqueado silenciosamente por políticas do browser moderno; substituído por modal React interno.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    pip       & ---  & Para instalar o \texttt{edge-tts} \\
    \bottomrule
  \end{tabular}
...
\begin{longtable}{L{3.5cm} L{3cm} L{7cm}}
  ...
  ORM                  & JPA/Hibernate + H2              & DDL automático em dev; migração para PostgreSQL/MySQL com uma linha de configuração \\
...
\begin{description}
  \item[Curto prazo]
    Mover a chave JWT para variável de ambiente; implementar \texttt{NotificationService} e \texttt{EmailService} reais; adicionar endpoint de health check para as dependências externas.
  \item[Médio prazo]
    Migrar a base de dados para PostgreSQL para produção; implementar \emph{lazy loading} de páginas React; substituir a cache in-memory por Redis para arquitetura distribuída.
  \item[Longo prazo]
    Suporte a múltiplas vozes TTS e idiomas; sistema de comentários em podcasts; integração com plataformas externas (Spotify, Apple Podcasts); deploy em cloud com Docker e CI/CD com deploy automático.
\end{description}

\section{Considerações Finais}

O Podcastia demonstra que é possível criar uma plataforma de media completa,
com funcionalidades sociais e conteúdo gerado por IA, utilizando tecnologias
\emph{open-source} ou gratuitas (Spring Boot, React, edge-tts) e um investimento
